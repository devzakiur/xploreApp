...
<?php echo $this->session->flashdata("msg"); ?>
        <?php  if(hasPermission("news_category",ADD)): ?>
            <?php if(isset($add)): ?>
                <div class="row">
                    <div class="col-sm-12">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="panel panel-border panel-info">
                            ...
                            <div class="panel-body">
                                <!-- <form id="find"> -->
                                <?php echo form_open("news/news_category_edit/".$single->id); ?>
                                    <div class="row">
                                        <div class="col-sm-4">
                                            <div class="form-group">
                                                <label for="name">Name</label><small class="req"> *</small>
...
<script>
    $(document).ready(function(){
			...
			function last_order_position() {
				  $.get("<?php echo base_url() ?>ajax/get_last_order_position?table_name=news_category", function(data, status){
						$("#position").val(data);
				  });
			}
        $("#category_add").on("submit",function(e){
            e.preventDefault();
            var url="<?php echo base_url() ?>news/news_category_add";
            $.ajax({
                url:url,
                type:"post",
                dataType:"json",
        ...
        {
            $.ajax({
                url:"<?php echo base_url() ?>news/news_category_view",
                type:"get",
                dataType:"json",
                data:{"user":"s"},
                success:function(data){
